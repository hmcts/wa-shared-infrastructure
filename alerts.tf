...
module "wa-camunda-task-unconfigured-exception-alert" {
  ...
  app_insights_query = "union traces, exceptions | where customDimensions[\"LoggingLevel\"] == \"WARN\" and customDimensions[\"Logger Message\"] contains \"Task could not be configured.\" | sort by timestamp desc"
