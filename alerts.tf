...
module "tm-camunda-task-uninitiated-exception-slack-alert" {
  ...
  app_insights_query         = "traces | project timestamp, msg=message | union (exceptions | project timestamp, msg=outerMessage) | where msg has_all (\"TASK_INITIATION_FAILURES There are some uninitiated tasks\") | parse kind=relaxed msg with * \"created: \" created_iso \"Z\" * | extend created_str = strcat(created_iso, \"Z\") | extend created = todatetime(created_str) | where isnotempty(created) and created >= ago(5h) | project timestamp, created, msg"
  custom_email_subject       = "Alert: A task could not be initiated in wa-${var.env}"
...
module "tm-camunda-task-unterminated-exception-slack-alert" {
  source   = "git@github.com:hmcts/cnp-module-metric-alert"
  location = var.location
...
module "tm-messages-find-problem-messages-slack-alert" {
  source   = "git@github.com:hmcts/cnp-module-metric-alert"
  location = var.location

  app_insights_name = "wa-${var.env}"

  alert_name                 = "tm-messages-find-problem-messages-slack-alert"
...
module "tm-api-task-reconfiguration-exception-slack-alert" {
  source   = "git@github.com:hmcts/cnp-module-metric-alert"
  location = var.location

  app_insights_name = "wa-${var.env}"

  alert_name                 = "tm-api-task-reconfiguration-exception-slack-alert"
...
module "tm-task-replication-problem-slack-alert" {
  source   = "git@github.com:hmcts/cnp-module-metric-alert"
  location = var.location

  app_insights_name = "wa-${var.env}"

  alert_name                 = "tm-task-replication-problem-slack-alert"
